        ...
        <div className="flex gap-4 mt-16 overflow-x-auto pb-4 no-scrollbar">
          {GALLERY_IMAGES.map((img, idx) => (
            <div 
              ...
              className={`relative min-w-[100px] md:min-w-[120px] h-20 md:h-24 rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 flex-shrink-0 bg-slate-200 ${
                currentIndex === idx ? 'ring-4 ring-primary-500 ring-offset-2' : 'opacity-60 hover:opacity-100'
              }`}
            >
              {!failedImages[idx] ? (
                <img 
